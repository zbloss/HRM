...
from scripts.data.metadata import (
...
def puzzle_hash(puzzle: dict):
    ...
    hashes = []
    for _example_type, example in puzzle.items():
...
def convert_single_arc_puzzle(
    ...
    if aug_count > 0:
        ...
        for _trial in range(ARCAugmentRetriesFactor * aug_count):
            ...
            def _map_grid(grid: np.ndarray, mapping=mapping, trans_id=trans_id):
...
def convert_dataset(config: DataProcessConfig):
    ...
    identifier_map = {}
    for _split_name, split in data.items():
        for _subset_name, subset in split.items():
            for group in subset:
                for puzzle in group:
